...
def preprocess(mha, out_dir, start, end, desc='mha to png'):  # out_dir should end with /
    for i in trange(start, end, desc=desc, leave=False):
        Image.fromarray(mha[i]).save(out_dir + ('odd/' if i%2==0 else 'even/') + str(i).zfill(4) + '.png')

def load_img(result, paths, start, end):
    for i in trange(start, end, desc='loading png', leave=False, disable=True):
...
def main(in_path):
    ...
    cmd = f'python dynamic_divide.py -n 40 -t 3 -v 150 -r {10 if n_frames>=960 else 11} -h 408 -w 728 --model weights/728_408_100000.pth -i /tmp/color/odd -m /tmp/mask/odd -o /tmp/odd'
    system(cmd); system('mv /tmp/odd/result/final_result_dynamic/*png /output/png/')
    cmd = f'python dynamic_divide.py -n 40 -t 3 -v 150 -r {10 if n_frames>=960 else 11} -h 408 -w 728 --model weights/728_408_100000.pth -i /tmp/color/even -m /tmp/mask/even -o /tmp/even'
    system(cmd); system('mv /tmp/even/result/final_result_dynamic/*png /output/png/')

    res = empty((n_frames, 720, 1280, 3), dtype=uint8)
    paths = sorted(glob('/output/png/*png'))
...
#    os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'expandable_segments:True'
    ...
    Path('/tmp/color/odd').mkdir(parents=True); Path('/tmp/mask/odd').mkdir(parents=True)
    Path('/tmp/color/even').mkdir(); Path('/tmp/mask/even').mkdir()
    Path('/output/png/').mkdir(exist_ok=True)
